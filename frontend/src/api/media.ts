...
export async function getTimeline(from?: string, to?: string) {
  ...
  if (to) params.to = to
  const { data } = await api.get<MediaListResponse>('/api/media/timeline', { params })
  return data
};
...
export async function uploadBulkMedia(files: File[]) {
  ...
  const { data } = await api.post('/api/media/upload-bulk', form);
  return data;
};
...
export async function deleteBulkMedia(ids: string[]) {
  const { data } = await api.delete('/api/media/bulk-delete', { data: { ids } });
  return data;
};
