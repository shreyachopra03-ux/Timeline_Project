...
//   const [selected, setSelected] = useState<string[]>([]) // Sahi Array State
//   // FIX 1: Initial state khali rakhne ki jagah default text de diya taaki manually type na karna pade
...
//   useEffect(() => {
//     getTimeline()
//       .then((m) => { setMedia(m.data); setLoading(false) })
//       .catch(() => setLoading(false))
//   }, [])

//   const handleGenerate = async () => {
//     // FIX 2: .size badal kar .length kiya
//     if (selected.length === 0) { toast('Select at least one media item.', 'error'); return }
//     if (!title.trim()) { toast('Enter a title.', 'error'); return }

//     setGenerating(true)
//     try {
//       // FIX 3: Direct selected bhej diya kyunki ye ab seedha array hai
...
//             // FIX 4: Title default milte hi aur item select hote hi button turant unlock ho jayega
...
//                   item={item}
//                   selectable
//                   // FIX 5: .has() hata kar .includes() lagaya array pattern ke liye
      ...
      <main className="max-w-6xl mx-auto pt-6 px-8 pb-6 space-y-6">
        ...
        >
          ...
          <div>
            ...
            {/* Fallback songs when no search results */}
